...
Based on: https://github.com/google/flax/blob/b60f7f45b90f8fc42a88b1639c9cc88a40b298d3/examples/imagenet/input_pipeline.py
"""
...
def create_split(dataset_builder, batch_size, train, dtype=tf.float32,
  ...
  Returns:
    A `tf.data.Dataset`.
  """
    name = dataset_builder.info.name

    if train:
        train_examples = dataset_builder.info.splits['train'].num_examples
        split_size = train_examples // jax.process_count()
        start = jax.process_index() * split_size
        split = 'train' if 'image_folder' in name else 'train[{}:{}]'.format(start, start + split_size)
    else:
        validate_examples = dataset_builder.info.splits['val'].num_examples if 'image_folder' in name else dataset_builder.info.splits['validation'].num_examples
        split_size = validate_examples // jax.process_count()
        start = jax.process_index() * split_size
        split = 'val' if 'image_folder' in name else 'validation[{}:{}]'.format(start, start + split_size)
