...
def scale_by_fosi(
    ...
    def _appprox_newton_direction(g1, state, params):
        ...
        k_learning_rates, k_eigenvecs, scaling_factor = jax.lax.cond(
            (state.count + 1 >= warmup_w) & ((state.count + 1 - warmup_w) % num_iters_to_approx_eigs == 0),
            lambda x: _approx_learning_rates_and_eigenvectors(x),
            lambda x: (state.k_learning_rates, state.k_eigenvecs, state.scaling_factor), params)
